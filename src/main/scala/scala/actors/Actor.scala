...
// $Id: Actor.scala 14894 2008-05-04 21:07:04Z phaller $
 ...
 */
@serializable
trait Actor extends OutputChannel[Any] with Logged {
  ...
  def send(msg: Any, replyTo: OutputChannel[Any]) = synchronized {
    tick()
    if (waitingFor(msg)) {
      log("Actor").debug("xxxxxxxx[actor] - {} - waiting for message", this)
      received = Some(msg)
      ...
      log("Actor").debug("xxxxxxxx[actor] - {} - not waiting for message. Putting message into mbox.", this)
   ...
   */
  def react(f: PartialFunction[Any, Unit]): Nothing = {
    log("Actor").debug("xxxxxxxx[actor] - {} - react running", this)
    assert(Actor.self == this, "react on channel belonging to other actor")
    if (shouldExit) exit() // links
    this.synchronized {
      tick()
      log("Actor").debug("xxxxxxxx[actor] - {} - react really running. Mbox size: {}", this, mailbox.size)
      val qel = mailbox.extractFirst((m: Any) => f.isDefinedAt(m))
      if (null eq qel) {
        log("Actor").debug("xxxxxxxx[actor] - {} - no matching msg found in Mbox", this)
        waitingFor = f.isDefinedAt
        continuation = f
        isDetached = true
      } else {
        sessions = List(qel.session)
        log("Actor").debug("xxxxxxxx[actor] - {} - matching msg found in Mbox; scheduling actor", this)
  ...
  private def seq[a, b](first: => a, next: => b): Unit = {
    ...
    log("Actor").debug("xxxxxxxx[actor] - {} - sequencing first body", this)
    first
    log("Actor").debug("xxxxxxxx[actor] - {} - sequencing: first done; throwing KillActorException", this)
